...
namespace optkit::core::frequency
{
    ...
    BaseGovernor::BaseGovernor(bool data_collector_mode, int64_t sample_period) : data_collector_mode{data_collector_mode}, config{false, true, false, 0, -1}, sample_period{sample_period}
    {
        ...
        this->config.cpu = Query::OPTKIT_SOCKET1__ENABLED ? Query::detect_cpu_packages().at(1)[0] : Query::detect_cpu_packages().at(0)[0];
    ...
    void BaseGovernor::call_back(int32_t signum, siginfo_t *oh, void *blah)
    {
        ...
        static int64_t max_core_freq = QueryCPUFreq::get_cpuinfo_max_freq();
        static int64_t min_core_freq = QueryCPUFreq::get_cpuinfo_min_freq();
    ...
    void BaseGovernor::read_scaler_file(std::vector<float> &mean_values, std::vector<float> &scale_values)
    {
        mean_values.clear();
        scale_values.clear();
        static std::string file_txt = optkit::utils::read_file("/usr/local/include/optkit_governor_model/scaler.txt");
