    ...
    public:
        ...
        static const MetricBuilder<uint64_t> &get_metric(const std::string &metric_name)
        {
            if (metric_name == "INST_RETIRED")
                return instructions();
            if (metric_name == "LLC-load")
                return llc_load();
            if (metric_name == "LLC-store")
                return llc_store();
        ...
        static const MetricBuilder<uint64_t> &llc_load()
        {
            static const MetricBuilder<uint64_t> metric = [] {
                const std::string event_name = riscv::to_string(riscv::NativeEvents::LLC_LOAD);
                return MetricBuilder<uint64_t>{}
                    .add(event_name, riscv::EventMapper::get(riscv::NativeEvents::LLC_LOAD))
                    .build(event_name,
                           [event_name](const std::unordered_map<std::string, uint64_t> &counts) -> double
                           {
                               return static_cast<double>(get_event_count(counts, event_name));
        ...
        static const MetricBuilder<uint64_t> &llc_store()
        {
            static const MetricBuilder<uint64_t> metric = [] {
                const std::string event_name = riscv::to_string(riscv::NativeEvents::LLC_STORE);
                return MetricBuilder<uint64_t>{}
                    .add(event_name, riscv::EventMapper::get(riscv::NativeEvents::LLC_STORE))
                    .build(event_name,
                           [event_name](const std::unordered_map<std::string, uint64_t> &counts) -> double
                           {
                               return static_cast<double>(get_event_count(counts, event_name));
