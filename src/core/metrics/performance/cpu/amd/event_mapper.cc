...
namespace optkit::metrics::performance::cpu::amd
{
        ...
        const std::unordered_map<metrics::performance::cpu::CoreEvents, std::vector<uint64_t>> EventMapper::core_event_map = {

            // Pipeline and Stalls
            {performance::cpu::CoreEvents::UNHALTED_CORE_CYCLES, {0x0076}},
            // {performance::::CoreEvents::UNHALTED_REFERENCE_CYCLES, {}}, // not exists
            {performance::cpu::CoreEvents::RESOURCE_STALLS, {0xffae}}, // focus on stall_1-> since it is directly related to resource like FP unit etc. (backend side of the performance::)
            // {performance::::CoreEvents::RECOVERY_CYCLES, {}},

            // Instruction Events
            {performance::cpu::CoreEvents::INST_RETIRED, {0x00c0}},
            ...
            // Branch Prediction
            {performance::cpu::CoreEvents::BRANCH_INST_RETIRED, {0x00c2}},
            {performance::cpu::CoreEvents::BRANCH_MISP_RETIRED, {0x00c3}},
...
// Cache Events
            ...
            {performance::cpu::CoreEvents::L1_MISSES, {0x5f44}},
        ...
        // {performance::::CoreEvents::L1_HITS, {}},
            ...
            {performance::cpu::CoreEvents::L2_MISSES, {0x0964, 0xff71, 0xff72}},
            {performance::cpu::CoreEvents::L2_HITS, {0xf664, 0xff70}},

            {performance::cpu::CoreEvents::L3_MISSES, {0x0104}},
            // {performance::::CoreEvents::L3_HITS, {}},  -- do L3_CACHE_ACCESSES - L3_MISSES

            // Memory Events
            {performance::cpu::CoreEvents::MEM_INST_RETIRED, {0x0729}},  // LS_DISPATCH, ldopst: single op that performs both load and store. purest: single op performs pure store. pureld: single op performs pure load.
            {performance::cpu::CoreEvents::MEM_LOAD_RETIRED, {0x0129}},  // LS_DISPATCH, pureld: single op only performs load (ldopst is disregarded.)
            {performance::cpu::CoreEvents::MEM_STORE_RETIRED, {0x0229}}, // LS_DISPATCH purest: signle op only performs store (ldopst is disregarded)

#if OPTKIT_ENV_CPU_MICROARCH_ZEN3 || OPTKIT_ENV_CPU_MICROARCH_ZEN4 || OPTKIT_ENV_CPU_MICROARCH_ZEN5
            {performance::cpu::CoreEvents::ITLB_MISSES, {0x0084, 0x0f85}},        // L1 ITLB misses (hits l2 or goes for page walk)
            {performance::cpu::CoreEvents::ITLB_MISSES_GOES_PAGE_WALK, {0x0f85}}, // L1 ITLB miss  (miss l2 and goes for page walk)
#else
            {performance::cpu::CoreEvents::ITLB_MISSES, {0x0084, 0x0785}},        // L1 ITLB misses (hits l2 or goes for page walk)
            {performance::cpu::CoreEvents::ITLB_MISSES_GOES_PAGE_WALK, {0x0785}}, // L1 ITLB miss  (miss l2 and goes for page walk)
#endif

            {performance::cpu::CoreEvents::DTLB_MISSES, {0xff45}},                // L1 DTLB miss (hits l2 or goes for page walk)
            {performance::cpu::CoreEvents::DTLB_MISSES_GOES_PAGE_WALK, {0xf045}}, // L1 DTLB miss (miss l2 and goes for page walk)
            {performance::cpu::CoreEvents::SW_LOAD_PREFETCH_ACCESS, {0xFF59}},
        ...
        // {performance::::CoreEvents::FP_ARITH_INST_RETIRED, 0x0},   // amd doesn't distinquish
            ...
            {performance::cpu::CoreEvents::RETIRED_VECTOR, {0xff03}},    // sse_avx
            {performance::cpu::CoreEvents::RETIRED_FLOPS_ANY, {0xff03}}, // fp_ret_sse_avx_ops.all -- Retired SSE and AVX floating-point ops of all types, counts packed and scalar ops.

#elif OPTKIT_ENV_CPU_MICROARCH_ZEN4
            {performance::cpu::CoreEvents::RETIRED_VECTOR, {0x1f03}},    // TODO: fix this, you need to seperate like intel to get all vector ops. fp_pack_ops_retired.all -- Retired packed floating-point ops of all types.
            {performance::cpu::CoreEvents::RETIRED_FLOPS_ANY, {0x1f03}}, // fp_ret_sse_avx_ops.all -- Retired SSE and AVX floating-point ops of all types, counts packed and scalar ops.

#elif OPTKIT_ENV_CPU_MICROARCH_ZEN5
            {performance::cpu::CoreEvents::RETIRED_VECTOR, {0x0f03}},    // sse_avx
            {performance::cpu::CoreEvents::RETIRED_FLOPS_ANY, {0x0f03}}, // fp_ret_sse_avx_ops.all -- Retired SSE and AVX floating-point ops of all types, counts packed and scalar ops.
#else
        ...
        const std::unordered_map<metrics::performance::cpu::amd::NativeEvents, std::vector<uint64_t>> EventMapper::native_event_map = {
            ...
            {performance::cpu::amd::NativeEvents::RETIRED_MICROCODE_OPS, {0x1000000c2}},
            {performance::cpu::amd::NativeEvents::DISPATCH_STALLS_1, {0x1000001a0}}, // Differs from PFM, exceeds 8 bits, but matches AMD docs and produces correct results with `perf stat`.
            {performance::cpu::amd::NativeEvents::DISPATCH_STALLS_1_0x6, {0x1060001a0}},
            {performance::cpu::amd::NativeEvents::BACKEND_STALLS_1, {0x100001ea0}}, // Differs from PFM, exceeds 8 bits, but matches AMD docs and produces correct results with `perf stat`.
            {performance::cpu::amd::NativeEvents::SMT_STALLS_1, {0x1000060a0}},     // Differs from PFM, exceeds 8 bits, but matches AMD docs and produces correct results with `perf stat`.
            {performance::cpu::amd::NativeEvents::OPS_SOURCE_DISPATCHED_FROM_DECODER, {0x07aa}},
...
#if OPTKIT_ENV_CPU_MICROARCH_ZEN3 || OPTKIT_ENV_CPU_MICROARCH_ZEN4 || OPTKIT_ENV_CPU_MICROARCH_ZEN5
            {performance::cpu::amd::NativeEvents::L3_CACHE_ACCESSES, {0xff04}},
            {performance::cpu::amd::NativeEvents::L2_CACHE_ACCESSES, {0xf960, 0xff70, 0xff71, 0xff72}},
#endif

#if OPTKIT_ENV_CPU_MICROARCH_ZEN5
            {performance::cpu::amd::NativeEvents::SCALAR_SINGLE_FLOPS, {0x4003}}, // fp_ret_sse_avx_ops.scalar_single -- Retired SSE and AVX scalar single-precision floating-point ops.
            {performance::cpu::amd::NativeEvents::PACKED_SINGLE_FLOPS, {0x6003}}, // fp_ret_sse_avx_ops.packed_scalar -- Retired SSE and AVX packed scalar floating-point ops.
            {performance::cpu::amd::NativeEvents::SCALAR_DOUBLE_FLOPS, {0x8003}}, // fp_ret_sse_avx_ops.scalar_double -- Retired SSE and AVX scalar double-precision floating-point ops.
            {performance::cpu::amd::NativeEvents::PACKED_DOUBLE_FLOPS, {0xa003}}, // fp_ret_sse_avx_ops.packed_double -- Retired SSE and AVX packed double-precision floating-point ops.
#endif
