#include "core/frequency/gpu/query.hh"

namespace optkit::frequency::gpu
{

    // Socket id - gpus belonging to that socket
    // e.g. {0: [0, 1, 2, 3], 1: [8, 9, 10, 11]} means socket 0 has cores 0-3 and socket 1 has cores 4-7
    static const std::map<int32_t, std::vector<int32_t>> &package_info = optkit::Query::detect_gpu_packages();
...
#define TRAVERSE_CORES(socket)                           \
    ...
    {                                                    \
        ...
        for (int32_t __gpu : package_info.at(socket))

    std::vector<int64_t> Query::get_scaling_available_frequencies(int32_t core)
    {
        std::vector<int64_t> frequencies;
        std::string avail_freqs = optkit::utils::read_file("/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_available_frequencies");
    ...
    int64_t Query::get_bios_limit(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/bios_limit";
    ...
    std::string Query::get_scaling_driver(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_driver";
    ...
    std::string Query::get_scaling_governor(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_governor";
    ...
    void Query::set_scaling_governor(const std::string &governor, int32_t socket)
    {
        try
        {
            ...
            std::string gov_path = "/sys/devices/system/gpu/gpu" + std::to_string(sample_core) + "/gpufreq/scaling_available_governors";
            ...
            {
                std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(__gpu) + "/gpufreq/scaling_governor";
    ...
    void Query::set_scaling_governor_percore(const std::string &governor, int32_t core)
    {
        try
        {
            std::string gov_path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_available_governors";
            ...
            // Apply governor
            std::string set_path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_governor";
    ...
    std::vector<std::string> Query::get_available_governors(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_available_governors";
    ...
    int64_t Query::get_scaling_max_limit(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_max_freq";
    ...
    int64_t Query::get_scaling_min_limit(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/scaling_min_freq";
            std::string content = optkit::utils::read_file(path);
            // Trim trailing newline
            if (!content.empty() && content.back() == '\n')
                content.pop_back();
    ...
    int64_t Query::get_gpuinfo_max_freq(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/gpuinfo_max_freq";
            std::string content = optkit::utils::read_file(path);
            // Trim trailing newline
            if (!content.empty() && content.back() == '\n')
                content.pop_back();
        ...
        {
            OPTKIT_CORE_ERROR("Failed to get gpuinfo max freq for core {}: {}", core, e.what());
            return -1;
        }
    }

    int64_t Query::get_gpuinfo_min_freq(int32_t core)
    {
        try
        {
            std::string path = "/sys/devices/system/gpu/gpu" + std::to_string(core) + "/gpufreq/gpuinfo_min_freq";
            std::string content = optkit::utils::read_file(path);
            // Trim trailing newline
            if (!content.empty() && content.back() == '\n')
                content.pop_back();
        ...
        {
            OPTKIT_CORE_ERROR("Failed to get gpuinfo min freq for core {}: {}", core, e.what());
            return -1;
        }
    }
