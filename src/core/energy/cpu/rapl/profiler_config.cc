...
namespace optkit::energy::rapl
{
    ...
    std::ostream &operator<<(std::ostream &os, const optkit::energy::rapl::RaplConfig &rapl_config)
    {
        ...
        os << "  " << optkit::energy::rapl::rapl_read_method_name_mapping.at((int32_t)rapl_config.read_method) << " ";

        if (rapl_config.monitor_domain & (int32_t)optkit::energy::rapl::RaplDomain::PP0)
        {
            os << "PP0 ";
        }
        if (rapl_config.monitor_domain & (int32_t)optkit::energy::rapl::RaplDomain::PP1)
        {
            os << "PP1 ";
        }
        if (rapl_config.monitor_domain & (int32_t)optkit::energy::rapl::RaplDomain::PACKAGE)
        {
            os << "PACKAGE ";
        }
        if (rapl_config.monitor_domain & (int32_t)optkit::energy::rapl::RaplDomain::PSYS)
        {
            os << "PSYS ";
        }
        if (rapl_config.monitor_domain & (int32_t)optkit::energy::rapl::RaplDomain::DRAM)
        {
            os << "DRAM ";
        }
        return os;
