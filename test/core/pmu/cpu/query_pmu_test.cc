...
{
    auto ids = Query::avail_pmu_ids();
    EXPECT_FALSE(ids.empty());
}

TEST(QueryPMUTest, DefaultPMUisPresent)
{
    pfm_pmu_info_t info = Query::default_pmu_info();
...
TEST(QueryPMUTest, EventDetailReturnsValidInfoForEachPMU)
{
    ...
    for (int32_t pmu_id : ids)
    {
        ...
        ASSERT_NO_FATAL_FAILURE(event_info = Query::event_detail(pmu_id, 0))
...
{
    pfm_pmu_info_t default_info = Query::default_pmu_info();
    pfm_pmu_info_t queried_info = Query::pmu_info(default_info.pmu);
